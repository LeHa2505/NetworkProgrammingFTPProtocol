...
void client_download(int sock, char *buffer, char *target_file) {
  if( access(target_file, F_OK ) == 0 ) { //target_file === path
    // file exists
...
void client_process(int sock, char *buffer, char **path) {
  ...
  char *delim = " ";
  char *command = strtok(full_command, delim); // first token
...
int main(int argc, const char *argv[]) {
  ...
  // Create Socket
  int sock; //clientfd for client; *file descriptors
  // int socket(int domain, int type, int protocol);
  if ((sock = socket(PF_INET, SOCK_STREAM, 0)) < 0) { //SOCK_STREAM => TCP, (PF_INET === AF_INET)
  ...
  char buffer[1024];
  char buff[MAX];
  bzero(&server_addr, sizeof(server_addr)); // set size of server_addr to O
  server_addr.sin_family = AF_INET;
  server_addr.sin_port = htons(atoi(argv[2])); 
  // printf("Hello: %d", atoi(argv[2]));
